...
AMP starts with all-zero estimate vector. At each iteration step, the estimate result \\(\hat{\boldsymbol{h}}^t\\) will be updated with applying a threshoulding function \\(\eta(\cdot)\\). The purpose of \\(\eta(\cdot)\\) function is to set small elements in the current estimate into zero to force sparsity.
