...
that is, $$y=\boldsymbol{A}\cdot h +w$$, where \\(w\\) is assumed to be Gaussian noise vector and \\(\boldsymbol{A} (n\times N)\\) is called sening matrix, composed of shift pilot sigals. Note that the size of \\(y\\) is \\(n \times 1\\), and the size of \\(x\\) and \\( h\\) are \\(N \times 1\\).
